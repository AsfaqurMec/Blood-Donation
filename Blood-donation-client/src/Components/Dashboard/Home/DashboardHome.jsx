...
useEffect(() => {
  if (donate.length > 0) {
    ...
    // Clear the timeout if the component is unmounted or request changes
...
//setTimeout(()=>{  setRemain(request);}, "500");
...
//console.log(totalPrice);
  ...
  const handleDelete = id => {
    // console.log(id);
     ...
     fetch(`https://blood-donation-green.vercel.app/delete/${id}`, {
         method: 'DELETE'
     })
         .then(res => res.json())
         .then(data => {
 ...
 const handleCancel = (id) => {
  ...
  fetch(`https://blood-donation-green.vercel.app/request/${id}`, {
      method: 'PUT',
      headers: {
...
//console.log(alluser);
const filter = alls.filter(item=> item.role == 'donor');
const users = filter.length;
//console.log(users);
//console.log(users);
//console.log(request);
const name = alls.find(item=> item.email === user?.email);
//console.log(name);
    const lastThreeDesc = remain.slice(-3).sort((a, b) => b - a); 
    return (
        <div>
            <h1 className='mb-10 text-2xl'>Welcome, <span className='text-green-500 font-semibold text-4xl'>{name?.name || user?.displayName}</span> </h1>
            ...
            <div>
          {
           ...
           <div>
            ...
            <Link to='/dashboard/my-dontion-requests'> <button className='btn btn-success my-10'>View My All Request</button></Link>
             ...
             : <div>
              <h1 className="text-center mt-8 font-semibold text-2xl">No data. Please create request.</h1>
