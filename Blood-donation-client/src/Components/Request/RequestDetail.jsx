...
const RequestDetail = () => {
  ...
  //  console.log(id);
    ...
    const { data: uder = [],refetch } = useQuery({
      queryKey: ['uder'],
      queryFn: async () => {
          const res = await axiosSecure.get(`/user/${h}`);
          return res.data;
      }
    })
...
console.log(uder);
  ...
  // console.log(request);
    ...
    const handleAdd = (e) => {
        ...
        const donorName = uder?.name;
      
        const donorEmail = uder?.email;
        ...
        fetch(`https://blood-donation-green.vercel.app/donate/${_id}`, {
            method: 'PUT',
            headers: {
