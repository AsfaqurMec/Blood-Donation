...
const Search = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch(
                `https://blood-donation-green.vercel.app/api/filter?dictrict=${dictrict}&upazila=${upazila}&blood=${blood}`
            );
    ...
    return (
        ...
        <div className='flex flex-col w-[80%] p-6 rounded-md sm:p-10 bg-gray-100 text-gray-900'>
          ...
          <form onSubmit={handleSubmit} className='space-y-6'>
            ...
            <select id='blood' name="blood" value={blood} onChange={handleInputChange}  className="select select-bordered w-full max-w-xs">
  ...
  <option value="O-">O-</option>
    ...
    <div>
            ...
            <select id='dictrict' name="dictrict" value={dictrict} onChange={handleInputChange}  className="select select-bordered w-full max-w-xs">
...
<option value="Netrokona">Netrokona</option>
    ...
    <div>
            <label>Upazila :</label>
            <select id='upazila' name="upazila" value={upazila} onChange={handleInputChange}  className="select select-bordered w-full max-w-xs">
  ...
  <option value="Matiranga">Matiranga</option>
      ...
      <h1 className="text-center text-green-700 text-3xl font-semibold mb-6">Result</h1>
            ...
            <table className="table rounded-none bg-[#c3b8cbc1]">
                ...
                <thead>
                    <tr>
                        ...
                        <th className="px-[5px] md:px-3 text-stone-950 text-lg font-bold">Email</th>
                        <th className="px-[5px] md:px-3 text-stone-950 text-lg font-bold">Blood</th>
                        <th className="px-[5px] md:px-3 text-stone-950 text-lg font-bold">Upazila</th>
                        <th className="px-[5px] md:px-3 text-stone-950 text-lg font-bold">District</th>
             ...
             
                    {
                     results.map(user => 
                        ...
                        <td className="px-[5px] md:px-3 font-bold text-lg">{user.email}</td>
                         <td className="px-[5px] md:px-3 font-bold text-lg">{user.blood}</td>
                         <td className="px-[5px] md:px-3 font-bold text-lg">{user.upazila}</td>
                         <td className="px-[5px] md:px-3 font-bold text-lg">{user.dictrict}</td>
                    </tr>
                      )
                    } 
                </tbody>
